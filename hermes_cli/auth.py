...
def _poll_for_token(
    ...
    raise TimeoutError("Device authorization timed out before the token "
                       "endpoint returned a decision. Please try again.")
...
# Same invalidation contract as the auth-status memo — the global auth
...
def _prompt_model_selection(
    ...
    if _unavailable and portal_url:
...
# deliberate no-ops so such entries simply fail their refresh and retire.

def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    """Nous OAuth was removed — no refresh error is treated as terminal."""
    return False


def _quarantine_nous_oauth_state(*_args, **_kwargs) -> None:
    """Nous OAuth was removed — nothing to quarantine."""
    return None


def _quarantine_nous_pool_entries(*_args, **_kwargs) -> None:
    """Nous OAuth was removed — nothing to quarantine."""
    return None
