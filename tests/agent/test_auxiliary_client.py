...
class TestGetProviderChain:
    ...
    def test_returns_three_entries(self):
        """The auto chain is openrouter → custom → api-key (nous rung removed)."""
        from agent.auxiliary_client import _get_provider_chain
        chain = _get_provider_chain()
        assert len(chain) == 3
        assert [label for label, _ in chain] == ["openrouter", "local/custom", "api-key"]
...
class TestAuxUnhealthyCache:
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # Mark the failed provider (openrouter) unhealthy; the chain should
        # still find the api-key provider.
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(api_client, "k-model")):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
